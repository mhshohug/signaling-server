...
function initSocket(io) {
  io.on('connection', (socket) => {
    ...
    socket.on('client_touch_move', (data) => {
      if (data && data.roomId) {
        socket.to(data.roomId).emit('partner_touch_move', data);
      } else if (data && (data.receiverId || data.targetUserId)) {
        const targetUser = onlineUsers.get(data.receiverId || data.targetUserId);
        if (targetUser) {
          io.to(targetUser.socketId).emit('partner_touch_move', data);
        }
      }
    });

    socket.on('client_touch_end', (data) => {
      if (data && data.roomId) {
        socket.to(data.roomId).emit('partner_touch_end', data);
      } else if (data && (data.receiverId || data.targetUserId)) {
        const targetUser = onlineUsers.get(data.receiverId || data.targetUserId);
        if (targetUser) {
          io.to(targetUser.socketId).emit('partner_touch_end', data);
        }
      }
    });

    
    socket.on('mark_for_deletion', (data) => {
      // Broadcast delete_message_confirmed to the room or both users
      if (data && data.roomId) {
        io.in(data.roomId).emit('delete_message_confirmed', data);
      } else {
        const receiver = onlineUsers.get(data.receiverId || data.targetUserId);
        const sender = onlineUsers.get(data.senderId || data.userId || socket.userId);
        if (receiver) io.to(receiver.socketId).emit('delete_message_confirmed', data);
        if (sender) io.to(sender.socketId).emit('delete_message_confirmed', data);
      }
    });

        socket.on('delete_love_message', async (data) => {
      const roomId = data?.roomId || data?.room_id;
      if (roomId) {
        io.in(roomId).emit('delete_love_message', data);
      } else {
        const receiverId = data?.receiverId || data?.receiver_id || data?.targetUserId || data?.target_user_id;
        const receiver = receiverId ? onlineUsers.get(receiverId) : null;
        if (receiver) {
          io.to(receiver.socketId).emit('delete_love_message', data);
        }
        socket.emit('delete_love_message', data);
      }
      const messageId = data?.messageId || data?.id || data?.message_id;
      if (messageId) {
        try {
          await supabase.from('messages').delete().eq('id', messageId);
          logger.info(`Deleted love message ${messageId} from Supabase messages table`);
        } catch (err) {
          logger.error(`Error deleting love message ${messageId}: ${err.message}`);
        }
      }
    });
