  ...
  io.on('connection', (socket) => {
    ...
    for (const invitation of activeCallInvitations.values()) {
      if (invitation.targetUserId === userId) {
        const callerUser = onlineUsers.get(invitation.callerId);
        const cName = callerUser ? callerUser.fullName : 'User';
    ...
    socket.on('call_user', async (data) => {
      ...
      // Trigger high-priority FCM notification only if target is offline or backgrounded
      if (!targetUser) {
        ...
        logger.info(`Receiver ${targetUserId} is active online via Socket.IO. Suppressing FCM call notification.`);
      }
    });
